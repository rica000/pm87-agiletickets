...
public class Espetaculo {
	...
	public List<Sessao> criaSessoes(LocalDate inicio, LocalDate fim,
		...
		if (periodicidade == Periodicidade.DIARIA) {
			numDeSessoes = Days.daysBetween(inicio, fim).getDays() + 1;
		} else {
			numDeSessoes = Days.daysBetween(inicio, fim).getDays();
			System.out.println(numDeSessoes);
			if (numDeSessoes <= 7) {
				numDeSessoes = 1;
			} else {
				numDeSessoes = (numDeSessoes / 7) + 1;
			}
		}

		for (int i = 1; i <= numDeSessoes; i++) {
